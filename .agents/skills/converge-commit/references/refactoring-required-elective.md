...
4. if all viable fixes are cross-boundary, keep commit minimal and escalate
   using `refactor_plan_suggestions`
...
2. chosen Chapter 6 actions -> `reduction_decisions`
3. unresolved cross-boundary debt -> `remaining-risk`
4. dedicated follow-up proposal -> `refactor_plan_suggestions`
