...
- 本地录音文件需要可显式清理，且清理动作不能依赖 SSH 控制通道。
...
- 清空录音文件通过本地 recorder 执行；动作仅影响 `recordings/` 目录下录音产物，不经过远端控制通道。
...
9. UI 可触发“清空录音文件”，并清除本地录音目录中的录音产物。
...
- [x] 已补齐本地录音文件清理入口：左侧栏可直接触发 recorder 清理。
