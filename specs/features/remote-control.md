...
- 应用窗口关闭时需要确定性停止本地 listeners，避免后台回调命中已销毁 UI bridge。
...
7. 正常应用退出会停止本地监听线程，且不会隐式停止远端 odaslive。
8. “清空日志”成功后，新增日志到达时 UI 必须继续更新，而不是停留在空态。
